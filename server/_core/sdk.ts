...
class SDKServer {
  ...
  async signSession(
    ...
    const expiresInMs = options.expiresInMs ?? (process.env.NODE_ENV === 'production' ? 30 * 24 * 60 * 60 * 1000 : ONE_YEAR_MS);
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    // 2. Fallback: Authorization header (for WebViews where cookies don't persist)
    if (!sessionCookie) {
